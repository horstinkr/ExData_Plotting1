plot_all <- function() {
	...
	# create 4 plots in one overview:
	par(bg = "white")
	par(mfrow = c(2,2))

	# plot left top graph:
	plot(DT$Global_active_power ~ DT$Date2, type = "l", main="", xlab="", ylab="Global Active Power")
	
	# plot right top graph:
	plot(DT$Voltage ~ DT$Date2, type = "l", main="", xlab="datetime", ylab="Voltage")
	
	# plot left bottom graph:
	plot(DT$Sub_metering_1 ~ DT$Date2, type = "n", main="", xlab="", ylab="Energy sub metering")
	with (DT, points(Sub_metering_1 ~ Date2, col="black", type = "l") )
	with (DT, points(Sub_metering_2 ~ Date2, col="red", type = "l") )
	with (DT, points(Sub_metering_3 ~ Date2, col="blue", type = "l") )
	legend("topright", pch="", lwd=2, cex=.8, bty = "n", col=c("black", "red", "blue"), legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

	# plot right bottom graph:
	plot(DT$Global_reactive_power ~ DT$Date2, type = "l", main="", xlab="datetime", ylab="Global_reactive_power")
